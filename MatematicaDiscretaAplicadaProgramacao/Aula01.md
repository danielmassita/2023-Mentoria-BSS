...
- Podemos representar o mesmo princípio e fim (abstração) da primeira bolinha (a esquerda) e simplificar as massas de terra (se não for essencial), então simplificamos e/ou abstraímos. As massas de terra não devem ser um problema, pois a questão proposta foi em relação às pontes (no caso, as linhas dos grafos). Por exemplo, a rota mais rápida do Google Maps que usa essa estrutura pra traçar rotas.
  ...
  - > O esforço pra "decorar" é mal gasto... Muito melhor entender o "por quê" foi criada... A "equação da freada" usada pelo Detran na perícia é a relação entre velocidade inicial e final, com o deslocamento durante a freada: (V)² - (Vzero)² = 2 * a * (Delta S). Entender "o por quê" das coisas... E vamos tentar quebrar a "maldição da matemática". Se não for explicado racionalmente, não vamos armazenar os motivos no cérebro... Aquilo que recebemos como "dogma" vamos traduzir para "entendimentos".
  - > _"Ed, como a matemática discreta aplicada e a ciência de dados (da aula de ontem) se correlacionam, na prática?"_ Consideremos a amostragem da estatística, consideremos a plotagem dos dados num gráfico (histograma, um gráfico de freqüência), são exemplos de matemática discreta, pois reduzimos o espaço amostral a um número discreto (categoria 1, cat. 2, cat. 3 de objetos). Por exemplo, objetos que funcionam e não-funcionam em uma amostra (on/off). São exemplos de matemática discreta. Em CD, podemos estabelecer coeficientes de correlação, modelando uma curva (splines, na modelagem, cúbicos, quadráticos, bi-quadráticos, etc.). O cálculo pra achar os coeficientes dos splines é um cálculo matricial com coeficientes inteiros (ou seja, matemática discreta). O próprio fato de estudarmos álgebra linear, representamos um volume ou plano, por dois vetores unitários (perpendiculares), mas são vetores unitários de tamanho 1, usamos como BASE pra multiplicar os vetores um vetor de escala 1, que é um número inteiro (seria discreta). Pegamos um conjunto de dados, usando métodos discretos ou não, pra a partir disso poder prever outros dados que virão (predição, tendência, etc.)
  - > Para os jovens, dos anos 2000 pra cá, houve um assassinato da matemática nas escolas. Separaram, pra efeitos didáticos, a matemática foi quebrada em grupos distintos (artificial) que só piorou o entendimento dos alunos do Ens. Fund. e Médio... Nós vamos estudar **Programação**, usando matemática como ferramenta!
  - > _"A matemática é uma descoberta ou uma invenção?"_ Pro Ed, é invenção. Mas essa resposta é muito ampla e profunda... Kurt Gödel deu uma rasteira na matemática no séc. XX: _"É comumente dito que Gödel demonstrou que existem verdades que não podem ser provadas. Tal afirmação está apenas parcialmente correta, pois não há nada no teorema de incompletude que afirme que uma sentença não pode ser demonstrada em um sentido absoluto."_ Existem problemas que não podem ser expressados em problemas matemáticos, e quando pudermos, a expressão do problema poderá ser mais profunda... Douglas Adams (série Guia do Mochileiro das Galáxias) diz que há muitas pessoas que acreditam que cada vez que alguém entende todo o universo, o sistema dá um re-boot e começa tudo de novo... Cada nova resposta abre novas dúvidas... Quando jovem, temos certezas, agora velhos, só temos dúvidas... 

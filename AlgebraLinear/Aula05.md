...
### Considerando "multiplicar por -1"

- Considerando o Conjunto V dos Vetores (no Espaço Vetorial) e também a mesma reta suporte do Corpo F (escalares), temos que (Conj. V) = (Conj. F).
- No eixo.x = V = F, pensemos num escalar de -2, então vamos dobrar o tamanho do vetor, porém o sinal (-) transforma E inverte o sentido, dobrando o tamanho e também invertendo o sentido. Logo, ao inverter o escalar (k = -2).
- Assim, um Vetor = 2, ao ser multiplicador por escalar = -2, vamos DOBRAR e INVERTER, levando o vetor de (0) para (-4), na reta V.
- Quando multiplicamos por -1, na verdade estamos invertendo o sentido a fim de anular o deslocamento, ou seja, vamos andar mas sem nos deslocar, então de ( 0 |-- 1 ), se multiplicarmos por -1, na verdade teremos um novo vetor de um para zero ( 0 --| 1 ). Ou seja, mesmo Módulo = |1| = |-1|, mesma Direção/Inclinação (ângulo θ cte.) na mesma reta suporte, porém com Sentidos opostos (antes 0|--1 e agora 1|--0). Esse deslocamento é, ao final, nulo.
- Então, temos de inverter, quando multiplicamos por um sinal negativo, pra mudar o Sentido e poder levar em conta essa transformação que vai me levar ao mesmo ponto de onde eu saí.
